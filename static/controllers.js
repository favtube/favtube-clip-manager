        ...
        function loadMoreClips(cb, initCall) {
            $http.post('/ajax/random', {
                ...
                .success(function(data) {
                    _.each(data, function(clip) {clip.id = idSeq ++});
...
//            $scope.userScroll(false);
        ...
        $scope.playClip = function(clip) {
            ...
            _.each($scope.clips, function(c) {c.selected = false});
            clip.selected = true;

            var jvideo = $('#player video');
            setTimeout(function() { jvideo[0].play(); } , 0);
            ...
            // play next
            if (!jvideo.data('event-inited')) {
                jvideo
                    .data('event-inited', true)
                    .on('ended', function() {
                        navTo('next');
                    });
            }
        ...
        $(function() {
            var $content = $('#clip-list .list-content').on('scroll', function() {
                if ($scope.mode != 'story') {
                    var cont = $content[0];
